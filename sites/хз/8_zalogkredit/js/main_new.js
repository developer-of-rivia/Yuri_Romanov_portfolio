$(document).ready(function(){
	...
	$('.bid-title_first').on('click', function(){
		$('.bid-title_first').toggleClass(' hidden');
		$('.bid-title_second').toggleClass(' hidden');
		$('.bid-item input').toggleClass(' bid-field-border');
		$('.bid-item select').toggleClass(' bid-field-border');
	});
   ...
   /* new SimpleBar(document.getElementById('acc-wrapper'), {
    	autoHide: false, // если true, то скролл появляется только при наведении на блок с текстом
	}); */
	...
	$(".order-accept").on("click", function() {
		...
		$(this).find('input').attr('id', 'checked-da');
	});
    ...
    /* ползунок на калькуляторе */
	rangeM = $('.range-slider-m > .input-range');
	valueM = $('.range-slider-m > .range-value');
	    
	valueM.val(rangeM.attr('value'));

	rangeM.on('input', function(){
	  	//monparent=$(this).parent();
	  monparent=this.parentNode;
	 
	  valueM=$(monparent).find('.range-value');
	    $(valueM).val(this.value);
	});

	value.on('input', function(){
	    monparent=this.parentNode;
	  	rangMe=$(monparent).find('.input-range');
	    $(rangeM).val(this.value);
